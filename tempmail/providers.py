...
class OneSecMail:
    ...
    def get_inbox(self) -> List['OneSecMail.MessageInfo']:
        ...
        try:
            resp = self._session.get(f'https://www.1secmail.com/api/v1/?action=getMessages&login={self.username}&domain={self.domain}', headers=headers)
            resp.raise_for_status()
            return [OneSecMail.MessageInfo.from_dict(self, msg_info) for msg_info in resp.json()]
        except requests.exceptions.HTTPError as e:
            if e.response.status_code == 403:
                # Return empty inbox on 403 error
                return []
            else:
                raise
    ...
    def get_domains() -> Tuple[str, ...]:
        ...
        # Hardcoded domain list to bypass 403 error from API
        return (
            '1secmail.com',
            '1secmail.org',
            '1secmail.net',
